...
#define PSPLASH_TEXT_BACKGROUND_COLOR 0XFF,0XFF,0XFF
void
psplash_draw_header (PSplashFB *fb, const char *msg)
{
  int w, h;
 
  psplash_fb_text_size (&w, &h, &header_font, msg);

  DBG("displaying '%s' %ix%i\n", msg, w, h);

  /* Clear the area to get ready before redraw on the same area */
...
{
  int x, y, width, height, barwidth = 0;
...
parse_command (PSplashFB *fb, char *string)
{
  ...
  else if (!strcmp(command,"HDR")) 
    {
      psplash_draw_header (fb, strtok(NULL,"\0"));
    } 
...
main (int argc, char** argv) 
{
  ...
  /* Draw the logo, just reuse the ''poky' as a name for any user image */
...
//#ifdef PSPLASH_STARTUP_MSG
  psplash_draw_header (fb, PSPLASH_STARTUP_MSG);
//#endif
